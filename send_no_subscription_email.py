...
SUBJECT = "-20% sur ton premier mois Trakx"
PREHEADER = "Code RETOUR20, valable cette semaine."

def email_html(first_name: str) -> str:
  ...
  <tr><td align="center" style="padding:32px 16px;">
    <table width="540" cellpadding="0" cellspacing="0" style="max-width:540px;width:100%;background:#ffffff;border-radius:16px;overflow:hidden;">
      ...
      <tr><td style="padding:20px 32px 0;font-size:16px;line-height:1.6;color:#111111;">
        ...
        <p style="margin:0 0 14px;">T'as créé un compte Trakx... mais jamais pris d'abonnement.</p>
        <p style="margin:0;color:#52525b;">Pendant ce temps, d'autres scannent <strong style="color:#111111;">450 000+ annonces Vinted/mois</strong> et trouvent les bonnes affaires avant tout le monde — sans rien faire.</p>
      </td></tr>
      ...
      <tr><td align="center" style="padding:24px 32px 0;">
        <table cellpadding="0" cellspacing="0">
          <tr><td style="background:#f97316;border-radius:10px;">
            <a href="https://trakx.fr/app/?promo=RETOUR20" style="display:block;padding:14px 36px;font-size:15px;font-weight:700;color:#ffffff;text-decoration:none;">Voir les plans →</a>
          </td></tr>
        </table>
      </td></tr>
    ...
    <table width="540" cellpadding="0" cellspacing="0" style="max-width:540px;width:100%;">
      <tr><td style="padding:16px 32px;" align="center">
        <p style="margin:0;font-size:11px;color:#a1a1aa;">
          Tu reçois cet email car tu t'es inscrit(e) sur <a href="https://trakx.fr" style="color:#a1a1aa;">trakx.fr</a>.
          &nbsp;·&nbsp;
...
def main():
    if TEST_MODE:
        ...
        ok = send_email(TEST_EMAIL, email_html("Jean"))
    ...
    targets = [u for u in confirmed if u["email"].strip().lower() not in subscribed_emails]

    print(f"📊 Détail : {len(users)} comptes au total")
    print(f"   ✅ {len(confirmed)} confirmés")
    print(f"   💳 {len(subscribed_emails)} déjà abonné(s)/en essai — exclus")
    print(f"   🎯 {len(targets)} ciblé(s) (jamais abonnés)")
    print()
    ...
    for i, user in enumerate(targets, 1):
        email = user["email"]
        success = send_email(email, email_html(extract_name(user)))
